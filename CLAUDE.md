...
- **[Spreadsheet Engine](docs/spreadsheet-engine.md)** (`hiveflow-connectors` engine + `hiveflow-portal` UI) — turns uploaded `.xlsx` workbooks into governed `silver/reference/` entities via parse → profile → interpret (Bedrock) → propose/approve → materialize, then DNA proposes lake joins deterministically (no AI) from grain/keys.
  - The **interpret + synthesize** stages use the vendored `hiveflow-core` / `hiveflow-spreadsheet-parser` packages (Claude Agent SDK). `spreadsheet/_agent_runtime.py` picks the backend at call time: `sdk` (multi-turn, tool-capable — needs Node + the `claude` CLI, only present in the interpret/propose **container-image** Lambdas built from `infra/agent_image/Dockerfile`; selected by `HIVEFLOW_AGENT_RUNTIME=sdk`) or `converse` (boto3 single-shot — the default everywhere else, incl. the portal Lambda's `synthesize_from_clean_goal` and local dev). Set `HIVEFLOW_AGENT_RUNTIME=sdk` + have `claude` on `PATH` to exercise the agent path locally via `jobs.run_pipeline`. All three tiers (agent → single-shot → heuristic) emit the same `report["tables"]` dict shape, so `materialize.py` and the DNA join proposal are unaffected.
