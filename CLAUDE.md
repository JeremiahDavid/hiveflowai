...
| `hiveflow-portal` | Portal shell UI, Cognito auth, charts, reporting surfaces (`src/hiveflow/dna/web/`), DNA Engine UI (`src/hiveflow/dna_engine/web/`, its own FastAPI app), Spreadsheet Engine UI (`src/hiveflow/spreadsheet_lab/web/`, its own FastAPI app) | dna + platform |
...
Two governed authoring flows build on this layered contract and are documented in depth — read the linked doc before working on either. Both are **agent apps**: each is its own FastAPI app/Lambda/subdomain trusting the portal shell's session cookie rather than being mounted in-process — see [dna-engine.md](docs/dna-engine.md)'s "Adding the next agent app" section before building a third one.
- **[DNA Engine](docs/dna-engine.md)** (`hiveflow-portal`, `hiveflow.dna_engine.web`, `/dna`, `/catalog`, `/governance`, `/semantics/source-docs`, its own subdomain — `ui.dna_engine.hostname` in `config.yaml`, default `dna-engine`) — catalog/pack browser, governance (compile/validate/publish/restore/versioning), Data Profile, Model Mapping, and the Source Browser. Split out of the portal shell's Werkzeug app (`PortalStack`, now Reporting Engine-only) into its own stack (`GlobalDnaEngineStack`, `infra/dna_engine.py`) so its Bedrock/Cognito-admin permission surface and blast radius are independent of client-facing reporting. Business logic (`hiveflow.dna.web.portal.{catalog,governance_helpers,data_profile_ui,model_mapping,semantics,kpi_generator}`) is reused unchanged from the former in-shell routes — only the FastAPI routing seam is new.
- **[KPI Generator](docs/kpi-generator.md)** (`/dna/kpi-generator` on the DNA Engine host) — NL-driven drafting of silver/gold Athena SQL, with a draft → integrity-validate → approve → publish governance workflow. Hosted inside DNA Engine, not a separate app.
