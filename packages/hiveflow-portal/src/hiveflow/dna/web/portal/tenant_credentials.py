"""Per-request tenant credential isolation.

The multi-tenant portal Lambda's own role can touch nothing tenant-scoped. Before
...
request. Credentials are cached per company until shortly before expiry.
...
def _new_tenant_session(company: str, environment: str) -> tuple[Any, float]:
    ...
    resp = sts.assume_role(
        ...
        RoleSessionName=f"portal-{company}"[:64],
