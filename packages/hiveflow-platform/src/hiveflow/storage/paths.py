...
def spreadsheet_engine_jobs_list_prefix() -> str:
    return f"{spreadsheet_engine_jobs_prefix()}/"


def spreadsheet_engine_job_tables_prefix(job_id: str) -> str:
    return f"{spreadsheet_engine_job_prefix(job_id)}/tables/"


def spreadsheet_engine_file_recipes_prefix() -> str:
    return f"{spreadsheet_engine_prefix()}/recipes/files"


def spreadsheet_engine_file_recipe_key(shape_hash: str) -> str:
    sid = shape_hash.strip().lower()
    if not sid or ".." in sid or "/" in sid:
        raise ValueError(f"Invalid file recipe shape hash: {shape_hash!r}")
    return f"{spreadsheet_engine_file_recipes_prefix()}/{sid}.json"


def spreadsheet_engine_table_recipes_prefix() -> str:
    return f"{spreadsheet_engine_prefix()}/recipes/tables"


def spreadsheet_engine_table_recipe_key(shape_hash: str) -> str:
    sid = shape_hash.strip().lower()
    if not sid or ".." in sid or "/" in sid:
        raise ValueError(f"Invalid table recipe shape hash: {shape_hash!r}")
    return f"{spreadsheet_engine_table_recipes_prefix()}/{sid}.json"
